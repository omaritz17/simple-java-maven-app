...
set -x
NAME=$(mvn -q -DforceStdout help:evaluate -Dexpression=project.name | tr -d '[:space:]')
set +x

echo 'Extracting the project version...'
set -x
VERSION=$(mvn -q -DforceStdout help:evaluate -Dexpression=project.version | tr -d '[:space:]')
set +x
